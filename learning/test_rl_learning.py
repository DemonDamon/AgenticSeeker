...
class MockInfoPool:
    ...
    def subscribe(self, info_type, handler):
        if info_type not in self.subscribers:
            self.subscribers[info_type] = []
        self.subscribers[info_type].append(handler)
    
    async def publish_info(self, info_type, data, priority=None):
        self.published_info.append({
            'type': info_type,
            'data': data,
            'priority': priority,
        ...
        if info_type in self.subscribers:
            for handler in self.subscribers[info_type]:
                try:
...
class MockKnowledgeManager:
    ...
    def __init__(self):
        self.knowledge_pool = MockKnowledgePool()
    
    class MockKnowledgePool:
        def __init__(self):
            self.knowledge_items = []
        
        async def add_knowledge(self, knowledge_item):
            self.knowledge_items.append(knowledge_item)
            print(f"添加知识项: {getattr(knowledge_item, 'id', 'unknown')}")
        
        async def query(self, **kwargs):
            # 返回模拟的知识项
            return self.knowledge_items[:kwargs.get('max_results', 10)]
...
class RLLearningTester:
    ...
    async def test_knowledge_integration(self) -> bool:
        """测试知识集成"""
        try:
            # 创建知识学习桥接器
            ...
            sync_result = await bridge.update_knowledge_from_experience(
                experiences, self.knowledge_manager.knowledge_pool
            )
            ...
            relevant_knowledge = await bridge.retrieve_relevant_knowledge(
                learning_context, self.knowledge_manager
            )
            ...
            print(f"  ✓ 知识集成测试通过，同步: {sync_result.items_synced} 项")
            return True
    ...
    async def test_rl_enhanced_engine(self) -> bool:
        """测试RL增强学习引擎"""
        try:
            # 创建RL配置
            ...
            # 测试启用RL模式
            rl_success = await engine.enable_rl_mode()
            assert rl_success
            ...
            assert 'rl_enabled' in rl_stats
            assert rl_stats['rl_enabled']
            
            # 关闭引擎
            await engine.shutdown()
            
            print(f"  ✓ RL增强学习引擎测试通过，会话ID: {result.session_id}")
            return True
            
        except Exception as e:
            print(f"  ✗ RL增强学习引擎测试失败: {e}")
            return False
